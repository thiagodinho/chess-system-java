	...
	public boolean[][] possibleMoves() {
		...
		if (getColor() == Color.WHITE) {
			...
			if(position.getRow() == 3) {
				...
				if(getBoard().positionExists(left) && isThereOponentPiece(left) && getBoard().piece(left) == cm.getEnPassantVulnerable()) {
					mat[left.getRow() - 1][left.getColumn()] = true;
				}
				Position right = new Position(position.getRow(), position.getColumn() + 1);
				if(getBoard().positionExists(right) && isThereOponentPiece(right) && getBoard().piece(right) == cm.getEnPassantVulnerable()) {
